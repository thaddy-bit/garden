...
export default function ProduitDetail({ product, produitsSimilaires, variantes = [], error }) {
  ...
  const ajouterAuPanier = async () => {
    ...
    try {
      ...
      if (res.ok) {
        // Mise à jour instantanée du compteur (optimiste)
        setCartCount(prevCount => prevCount + quantite);
        toast.success(`${quantite} article(s) ajouté(s) au panier !`);
        
        // Synchronisation en arrière-plan pour vérifier le vrai compteur
        try {
          const countRes = await fetch(`/api/panier/count?client_id=${user.id}`);
          if (countRes.ok) {
            const countData = await countRes.json();
            setCartCount(countData.total || 0);
          }
        } catch (error) {
  ...
  // Utiliser les prix de la variante sélectionnée ou du produit par défaut
  const prixFinal = selectedVariante?.prix_reduction || selectedVariante?.prix || product.prix_reduction || product.prix;
  const prixOriginal = selectedVariante?.prix || product.prix;
  const economie = selectedVariante?.prix_reduction ? selectedVariante.prix - selectedVariante.prix_reduction : 
                   product.prix_reduction ? product.prix - product.prix_reduction : 0;
  
  // Utiliser le stock de la variante sélectionnée ou du produit par défaut
  const stockDisponible = selectedVariante?.stock ?? product.stock;
              ...
              <div className="relative bg-gray-50">
                ...
                <div className="absolute top-4 left-4 z-10 flex flex-col space-y-2">
                  ...
                  {product.en_vedette === 1 && (
                    <span className="bg-gray-900 text-white px-3 py-1 rounded-full text-xs font-semibold shadow-lg">
                      ⭐ Vedette
                    </span>
                  )}
                  {product.en_solde === 1 && (
                ...
                <div className="absolute top-4 right-4 z-10 flex flex-col space-y-2">
                  ...
                  >
                    <svg className={`w-5 h-5 ${isFavorite ? 'text-green-600 fill-current' : 'text-gray-600'}`} viewBox="0 0 24 24">
                  ...
                  <button
                    ...
                    className="p-2 bg-white/90 backdrop-blur-sm rounded-full shadow-lg hover:bg-white transition-all"
                  >
                    <svg className={`w-5 h-5 ${isLiked ? 'text-green-600 fill-current' : 'text-gray-600'}`} viewBox="0 0 24 24">
                      <path d="M14 10h4.764a2 2 0 011.789 2.894l-3.5 7A2 2 0 0115.263 21h-4.017c-.163 0-.326-.02-.485-.06L7 20m7-10V5a2 2 0 00-2-2h-.095c-.5 0-.905.405-.905.905 0 .714-.211 1.412-.608 2.006L7 11v9m7-10h-2M7 20H5a2 2 0 01-2-2v-6a2 2 0 012-2h2.5"/>
                    </svg>
                  </button>
                </div>

                {/* Image principale - Prend tout le cadre */}
                      ...
                      {product.images.map((img, i) => (
                        <button
                          ...
                          className={`flex-shrink-0 w-16 h-16 rounded-lg overflow-hidden border-2 transition-all ${
                            selectedImage === i ? 'border-green-600' : 'border-gray-200 hover:border-green-400'
                          }`}
                        >
                          <Image 
                            src={img.image_url} 
        ...
        <div className="space-y-6">
                  ...
                  <div className="flex items-center space-x-4">
                    <div className="flex items-baseline space-x-2">
                      ...
                      {(selectedVariante?.prix_reduction || product.prix_reduction) && (
                  ...
                  <div className="flex items-center space-x-4">
                    {stockDisponible > 0 ? (
                      <div className="flex items-center space-x-2">
                        ...
                        <span className="text-sm text-gray-600">
                          {stockDisponible > 10 ? 'En stock' : `Plus que ${stockDisponible} en stock`}
                        </span>
                      </div>
                    ) : (
                      <div className="flex items-center space-x-2">
                    ...
                    {(selectedVariante?.sku || product.sku) && (
                      <span className="text-sm text-gray-400">SKU: {selectedVariante?.sku || product.sku}</span>
                    )}
                  </div>
          ...
          <button
            onClick={ajouterAuPanier}
                        ...
                        className="flex-1 bg-green-600 text-white px-8 py-4 rounded-lg font-semibold hover:bg-green-700 disabled:bg-gray-400 disabled:cursor-not-allowed transition-colors flex items-center justify-center space-x-2"
          >
...
export async function getServerSideProps({ params }) {
  try {
    ...
    let variantes = [];
    try {
      const variantesRes = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'}/api/produits/slug/${slug}/variantes`);
      if (variantesRes.ok) {
        variantes = await variantesRes.json();
      }
    } catch (err) {
      console.log('Erreur lors du chargement des variantes:', err);
    }
